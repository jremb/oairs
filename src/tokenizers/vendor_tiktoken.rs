...
impl CoreBPE {
    ...
    fn _encode_native(&self, text: &str, allowed_special: &HashSet<&str>) -> (Vec<usize>, usize) {
        ...
        let regex = self._get_tl_regex();
        let mut encoded_text = vec![]; // Changed from `ret`, jremb
        ...
        (encoded_text, last_piece_token_len)
    }
...
impl CoreBPE {
    ...
    pub fn decode_single_token_bytes(&self, token: usize) -> Option<Vec<u8>> {
    ...
    pub fn token_byte_values(&self) -> Vec<Vec<u8>> {
        self.sorted_token_bytes.to_vec()
    }
}
