//! This module contains the Client struct and a few private helpers for building the client and sending a request.
//! The Client struct is the main point of entry for interacting with the endpoints of the OpenAI API.
use reqwest::header;
...
impl Client<Keyed> {
    ...
    /// * `size` - The [`Size`] of the image to generate. Size vairants are [`Size::Small`] (`256x256`),
    /// [`Size::Medium`] (`512x512`),
    /// and [`Size::Large`] (`1024x1024`). API defaults to [`Size::Large`].
    /// * `response_format` - The [`ResponseFormat`] of the image to generate. Format variants are
    /// [`ResponseFormat::Url`] and [`ResponseFormat::Base64`]. API defaults to [`ResponseFormat::Url`].
    /// * `user` - The unique id for a user, for detecting abuse.
    ///
    /// # Returns
    /// `ImageBuilder<Sendable>` that can be used to set optional parameters and execute the request by
    /// awaiting `send()`.
    ///
    /// Awaiting `send()` will return `Result<reqwest::Response, OairsError>`. A successful response can
    /// be deserialized into an [`Image`] that can be saved to a file using the `save_json()` method.
    ///
    /// # Example
    /// ```rust,no_run
    /// let key = get_key();
    ...
    /// ```
    pub fn create_image<P: Into<String>>(&self, prompt: P) -> ImageBuilder<Sendable> {
        ImageBuilder::create(&self.key, prompt)
    }
    ...
    pub fn delete_fine_tune_model(self, model: &str) -> Client<Delete> {
        Client {
            ...
            url: Some(format!(
                "{}/{}",
                URL.get(&Uri::Models).unwrap().clone(),
                model
            )),
            ..Default::default()
        }
    ...
    pub fn list_files(&self) -> Client<Gettable> {
        Client {
            ...
            url: Some(URL.get(&Uri::Files).unwrap().clone()),
            ..Default::default()
        }
    }
    ...
    pub fn list_fine_tunes(&self) -> Client<Gettable> {
        Client {
            key: self.key.to_owned(),
            url: Some(URL.get(&Uri::FineTunes).unwrap().clone()),
            ..Default::default()
        }
    }
    ...
    pub fn list_models(&self) -> Client<Gettable> {
        Client {
            key: self.key.to_owned(),
            url: Some(URL.get(&Uri::Models).unwrap().clone()),
            ..Default::default()
        }
    }
    ...
    ///
    /// 1. Deserialize to a [`FineTuneResultsFC`](crate::files::response::FineTuneResultsFC) -
    ...
    pub fn retrieve_fine_tune_info(&self, fine_tune_id: &str) -> Client<Gettable> {
        Client {
            ...
            url: Some(format!(
                "{}/{}",
                URL.get(&Uri::FineTunes).unwrap().clone(),
                fine_tune_id
            )),
            ..Default::default()
        }
...
impl_post!(Client<Cancel>);

// There is a macro for this, but currently this impl handles all the `GET` requests except for
// `list_fine_tune_events`. Because the endpoint for that method has an optional parameter, we
// use the version of this method. In the future, will replace this with the macro here too as 
// long as it proves reliable for the former.
// TODO: Switch to macro if it proves reliable for `list_fine_tune_events`.
impl<'a> Client<Gettable> {
    pub async fn send(&'a self) -> Result<reqwest::Response, OairsError> {
...
// There is a macro for this, but currently this is only time we post with form, so not using it.
impl Client<Sendable> {
    /// Executes the `POST` request for a form. Returns a `Result` with either a 
    /// [`Response`](https://docs.rs/reqwest/0.11.14/reqwest/struct.Response.html) or an
    /// `OairsError`.
    pub async fn send(&self) -> Result<reqwest::Response, OairsError> {
        let path = self.upload_filename.clone().unwrap();
...
// Macro for this, but this impl covers the two use cases right now, so not using it.
impl Client<Delete> {
...
// For help sorting the macro patterns, implementing the appropriate send method for each state of
// the client
#[allow(dead_code)]
...
pub(crate) fn build_client(key: &str) -> Result<reqwest::Client, OairsError> {
    ...
    // Using an unwrap here because I assume the OpenAI API won't generate a key that
    // uses non-visible ASCII characters
